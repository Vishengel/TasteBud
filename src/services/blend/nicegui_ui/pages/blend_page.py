...
from services.blend.server.app import create_blend
from services.blend.server.data_model import BlendParticipant, BlendRequest
...
class BlendPage(NiceGUIPage):
    ...
    async def _run_blend(self):
        ...
        request = BlendRequest(
            initiator=BlendParticipant(user_id=all_user_ids[0]),
            participants=[BlendParticipant(user_id=uid) for uid in all_user_ids[1:]],
            config=BlendConfig(playlist_name=playlist_name, target_size=target_size),
        )

        try:
            response = await create_blend(request)
        except HTTPException as exc:
            ui.notify(f"Blend failed: {exc.detail}", type="negative")
            return
