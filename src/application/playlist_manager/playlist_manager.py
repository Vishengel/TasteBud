...
class PlaylistManager:
    def __init__(self, spotify_client: SpotifyClient, user_id: str | None = None):
        ...
        self.playlists: dict[str, SpotifyPlaylist] = self._get_playlists_dict()
        self.name_to_id_map = {playlist.name: playlist.id for playlist in self.playlists.values()}

    def playlists_for_main_user(self) -> list[SpotifyPlaylist]:
    ...
    def get_playlist(self, playlist_id: str) -> SpotifyPlaylist:
        ...
        return SpotifyPlaylist.from_spotify_playlist_dict(playlist)

    def get_all_playlists_for_user_id(self, user_id: str) -> list[SpotifyPlaylist]:
        raw_playlists = self.spotify_client.fetch_all_playlists(user_id)
        return [SpotifyPlaylist.from_spotify_playlist_dict(playlist) for playlist in raw_playlists]

    def get_tracks_for_playlist_id(self, playlist_id: str) -> list[SpotifyTrack]:
        raw_tracks = self.spotify_client.fetch_tracks_for_playlist(playlist_id)
        return [SpotifyTrack.from_spotify_track_dict(track_dict) for track_dict in raw_tracks]
    ...
    def _find_playlist_by_id(self, playlist_id: str) -> SpotifyPlaylist:
