...
async def create_blend(body: BlendRequest) -> BlendResponse:
    ...
    all_user_ids = [p.user_id for p in all_participants]
    tracks = engine.create_blend(all_user_ids, body.config)
    description = build_blend_description(all_user_ids, body.config)
